...
|---|---|---|---|---|---|
| 1 | P1 | layout | `#tuning-tab` | The tuning tab is a **silent scroll region**: computed `overflow-y: auto`, `clientHeight: 334`, `scrollHeight: **535**` — **201px of content, 38% of the panel, is below the fold** with `scrollTop: 0` and no scrollbar visible in any of the 6 captured tuning frames, in any language. Its child `div.tuning-controls-panel` is `position: absolute` with `height: 520` inside that 334px box, so the overflow is structural, not incidental. | Do not solve this with a scrollbar — a 700x450 plugin editor should not scroll. Collapse the right column into the existing `TUNING LIBRARY` disclosure (it already has a `▼` toggle at `(670, 95)`) so only one of {library, generator, file I/O} is expanded at a time, and the column fits 334px. |
| 2 | P1 | layout | `#generator-section` | The scale generator — a whole feature — **cannot be seen or reached at the shipped frame**. Measured: the section is at `y = 384…538`; `#generator-content` starts at `y=412`, `#generator-inputs` at `y=451` and `button.generator-btn` at `y=509`, all past the tab's fold at `y=407`. The one strip inside the fold (`div.generator-header`, `y 384…403`) is painted over — `document.elementFromPoint(572, 392)` returns **`div.footer`**. Proof the form is live but invisible: three consecutive `handleGeneratorTypeChange` calls rebuild `#generator-inputs` (table in `## Coverage`) while producing **byte-identical screenshots**. | Move `#generator-section` above `#scala-buttons` in the right column so it lands inside `y 88…407`, and gate it behind the same disclosure as finding 1. Until then the generator is shipped-but-unreachable. |
| 3 | P1 | layout | `div.footer` vs `.tab-content` | `div.footer` is `(3,392) 694x55` at `z-index: 10`; every `.tab-content` is `(3,73) 694x334`, ending at `y=407`. The footer therefore **paints over the bottom 15px of every tab, on every tab, in every language**. It is not cosmetic: on TECHNIQUES with both custom groups revealed, `div.slider-group` "Dynamics: Start" occupies `y 389…413` and `#glissandoCustomSemitonesValue` sits at `y=403` — both under the footer. | Reduce `.tab-content` height to `319px` (ending at `y=392`) so the two never overlap, or give the footer `position: static` and let the tab flow above it. The 15px is currently paid for by whichever control happens to land last. |
...
**Severity counts: P1 = 3, P2 = 6, P3 = 5 (14 total).**
...
- **Was the generator ever reachable?** Findings 1–2 describe a feature that is fully built
  and fully invisible at the shipped frame. It is worth knowing whether the frame shrank after
  the generator was laid out, or whether it was never checked at 700x450 — the answer changes
  whether other panels need the same audit.
