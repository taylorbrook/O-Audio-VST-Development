...
|---|---|---|---|---|---|
| 1 | P1 | layout | `#instrument-tab` | **More than half the INSTRUMENT tab is invisible.** Computed `overflow-y: auto`, `clientHeight: 449`, `scrollHeight: **945**` — **496px, 52% of the content, below the fold** at `scrollTop: 0`, with no scrollbar rendered in any of the five captured instrument frames in any language. Four entire sections live down there: `div.section-title` measures at `y=624` (Advanced), `707` (Multi-Stage Envelope), `816` (Filter), `888` (Performance), `952` (Output) — in a **600px** frame. The brief specifies a **"Two-panel design (Main / Advanced tabs)"**; the shipped UI has three tabs (Instrument / Tuning / Effects) and has buried Advanced inside Instrument instead. | Restore the brief's structure: promote **Advanced** to a fourth tab carrying Multi-Stage Envelope, Filter, Performance and Output. `#instrument-tab` then holds Synthesis + Ensemble + Onsets in 449px with room to spare, and finding 3's 131px of dead space on EFFECTS stops being an outlier. |
...
| 3 | P2 | layout | `#effects-tab` | `#eqSection` — the last content on the tab — ends at `y = 335 + 76 = **411**`. The tab runs to `y=542`. That leaves **131 x 734px of empty cream, 29% of the tab's height**, on the largest frame in the batch. Meanwhile the INSTRUMENT tab is hiding 496px (finding 1). The plugin is simultaneously out of room and full of room. [`en__14-effects-tab.jpg`](shots/O-Bells/en__14-effects-tab.jpg) | Either give the four effect sections the space — grow each `76px` row toward `~105px` so the knobs, captions and readouts breathe — or move a section out of finding 1's hidden region into it. Do not leave a third of the tab blank. |
...
| 10 | P3 | affordance | `#interval-list` | `(48,122) 160x300` with `overflow-y: auto`, `clientHeight: 298`, `scrollHeight: **421**` — **123px hidden, roughly 3.5 of the 12 intervals**. The header reads `Intervals: 12` while nine rows are shown, and there is no fade, no scrollbar and no "…" cue in the captured frames. Its parent `div.tuning-panel` is `421px` tall, so the list's full content would fit if the box were allowed to grow. [`en__04-tuning-tab.jpg`](shots/O-Bells/en__04-tuning-tab.jpg) | Grow `#interval-list` to `height: 391px` (matching `div.tuning-intervals-column`) so all 12 rows show at 12-TET, and add a bottom fade for tunings with more degrees. |
...
**Severity counts: P1 = 2, P2 = 7, P3 = 6 (15 total).**
...
- **The tuning tab FITS.** `#tuning-tab` computes `scrollHeight == clientHeight == 449` — no
  ...
  O-Lyrica ships what is recognisably the same tuning panel and hides 201px of it below the
  fold. **This is the fix for O-Lyrica findings 1–2, already built and already working here.**
...
- **Was "Advanced" ever a tab?** Finding 1 shows four sections stranded below a fold in a
  panel the brief said should have been split into Main and Advanced tabs. Knowing whether
  the tab was removed or never built decides whether this is a regression or an unfinished
  item.
