...
must_haves:
  truths:
    - "Agent invocation fails fast with clear error when inputs are invalid"
...
<objective>
Implement contract validation protocol and create full specifications for missing agents.

Purpose: Validation ensures contracts are enforced, not just documented. Missing agent specs prepare the system for future expansion to cover workflow gaps.

Output: Contract validation skill, centralized changelog, and complete specs for all missing agents identified in the audit.
</objective>
...
<task type="auto">
  <name>Task 2: Create centralized changelog and update existing schemas</name>
  <files>
    .claude/schemas/CHANGELOG.md
  </files>
  <action>
Create centralized changelog for all schema changes per CONTEXT.md decision.
...
- Versioning policy documented
  ...
  <done>
Changelog created tracking all v1.0.0 schemas. Version policy documented.
  </done>
</task>

<task type="auto">
  <name>Task 3: Create missing agent specifications</name>
  <files>
    .planning/phases/01-agent-contracts/MISSING-AGENT-SPECS.md
  </files>
  <action>
...
- Implementation priority table
  ...
  <done>
Missing agents fully specified with contracts and boundaries. Ready for future implementation.
  </done>
</task>
...
<verification>
- [ ] contract-validation/SKILL.md exists with error format
...
2. Changelog tracks all v1.0.0 schemas
3. Every gap from audit has a complete agent spec
4. Specs include full JSON schemas (not just descriptions)
5. Ready for phase completion
</success_criteria>
