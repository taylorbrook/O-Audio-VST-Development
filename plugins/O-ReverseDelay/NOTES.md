...
- **2026-09-23 (v1.12.3):** Patch — UI only. **Count knob answers wheel and arrows** (nudge floored at one parameter step; 0.02 normalised was 0.28 of grainCount's step and snapped back). **Wheel:** horizontal-dominant events ignored, delta-scaled 1–4 nudges per event, one automation gesture per burst (closes 250 ms after the last tick). **Lit states** (`--btn-active`) solid `--green-dark`: cream text 2.3:1 → 7.2:1 on SYNC/FREEZE/MONO·STEREO/open gear/armed Delete; hover-help "On" 3.3 → 6.8:1. **Delete disabled + aria-disabled on factory presets** (was: arm, confirm, silent no-op). **Small text:** footer 2.5 → 7.7:1 (opacity dropped), subtitle 3.7 → 7.7:1, settings labels 3.7 → 6.8:1, tooltip titles 3.3 → 5.6:1. ui-stub now snaps to interval like juce/index.js. No geometry change.
- **2026-09-23 (v1.12.4):** Patch — behaviour-preserving C++ simplification (audit HIGH-01..05, MED-05, LOW-01, LOW-04, and the C++ half of MED-07). Changes: `kShippedNoOpTail`, the allpass length argument dropped, `resetLoopState()`, `WindowLut::integrateHalves()`, the `kNoteDivisions` table, `makePresetDialogResult()`/`firstStringArg()`, `CaptureBuffer::wrapIndex()`, and stale invariant comments corrected. Proven **bit-identical** by the new harness `--digest` mode (45 scenarios, `ALL ddee853f446b7fbb` before and after). 156/156 probes pass with identical measurement lines. The re-seeded factory presets hold identical values. Audit status table added: 8 done, 2 partial, 10 open, all of the remainder on the UI/JS side.
