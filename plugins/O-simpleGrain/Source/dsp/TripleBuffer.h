...
//
// O-simpleGrain Phase 2.3: copied VERBATIM from O-GrainScatter
// (Source/dsp/TripleBuffer.h). Used for the grain-event tap
// (TripleBuffer<GrainCloudFrame>) — the audio thread fills
// getWriteBuffer() then publish()es once per block; the Stage-3
// editor Timer read()s on the message thread.
template <typename T>
class TripleBuffer
{
public:
