...
|----|-------------|----------|--------|-------------|
| FUNC-01 | Excitation → resonator → material signal chain is the playable structure (energy injected, carried, then damped) | must | pending | stage-2 |
| FUNC-02 | Swappable excitation: Pluck (noise burst), Strike (impulse/mallet), Bow (sustained friction) | must | pending | stage-2 |
| FUNC-03 | Two-way resonator switch: String (Karplus-Strong/waveguide) and Modal (decaying sinusoids) | must | pending | stage-2 |
| FUNC-04 | Same exciters drive either resonator (hold exciter, swap resonator — and vice versa) | must | pending | stage-2 |
| FUNC-05 | 16-voice polyphony; plays chords and sustains | should | pending | stage-2 |
| FUNC-06 | Velocity scales excitation strength/brightness (model responds to playing dynamics) | should | pending | stage-2 |
| FUNC-07 | Concept-isolating preset tour (bright steel, muted nylon, koto/harp, struck bar, bell, bowed string) | should | pending | stage-4 |

### DSP (DSP)

| ID | Description | Priority | Status | Verified At |
|----|-------------|----------|--------|-------------|
| DSP-01 | Karplus-Strong string: N-sample delay + one-pole low-pass + feedback loop; pitch = SR ÷ N | must | pending | stage-2 |
| DSP-02 | Fractional-delay tuning (all-pass interpolation) so notes stay in tune across the keyboard | must | pending | stage-2 |
| DSP-03 | Material/damping = loop low-pass cutoff (Damping) + feedback gain (Decay), feedback clamped < 1 for stability | must | pending | stage-2 |
| DSP-04 | Modal resonator: bank of decaying sinusoids with per-mode frequency/amplitude/decay; higher modes quieter and faster-decaying | must | pending | stage-2 |
| DSP-05 | Inharmonicity stretches modal mode spacing from harmonic (bar) toward inharmonic (bell) | must | pending | stage-2 |
| DSP-06 | Waveguide string option (dual traveling-wave delays) enabling meaningful Excitation Position | nice | pending | stage-2 |
| DSP-07 | Material macro maps a single steel↔nylon axis onto co-moving (cutoff, feedback) | should | pending | stage-2 |
| DSP-08 | DC-safe, band-limited excitation (no click/buzz/alias); Bow drive stable and sustaining | must | pending | stage-2 |

### UI (UI)

| ID | Description | Priority | Status | Verified At |
|----|-------------|----------|--------|-------------|
| UI-01 | Single clear page, classroom/projector-readable, laid out as the signal flows (excitation → resonator → material → output) | should | pending | stage-3 |
| UI-02 | Animated loop/flow diagram reflecting ACTUAL circulating energy, re-skinning per resonator (KS loop / waveguide / modal stems) and visibly dampening each pass | must | pending | stage-3 |
| UI-03 | Live waveform + decay scope | should | pending | stage-3 |
| UI-04 | Live spectrum/spectrogram showing harmonics fade top-down (and inharmonic spacing in Modal) | must | pending | stage-3 |
| UI-05 | Modal stem display (mode frequencies + decay rates) in Modal mode | should | pending | stage-3 |
| UI-06 | On-hover pedagogical tooltips on every control | nice | pending | stage-3 |

### Performance (PERF)

| ID | Description | Priority | Status | Verified At |
|----|-------------|----------|--------|-------------|
| PERF-01 | Real-time safe audio processing (no allocations in processBlock); visualization data via lock-free FIFO | must | pending | stage-2 |
...
| ID | Description | Priority | Status | Verified At |
|----|-------------|----------|--------|-------------|
| QUAL-01 | No audio artifacts (no clicks, buzz, aliasing, or runaway feedback) across parameter ranges and the keyboard | must | pending | stage-2 |
