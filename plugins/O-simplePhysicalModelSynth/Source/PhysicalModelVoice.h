...
public:
    ...
    void stopNote (float /*velocity*/, bool allowTailOff) override
    {
        ...
        {
            // Hard stop (voice steal, CC120 All Sound Off, releaseResources): the
            // env was noteOn'd with sustain=1, so without a reset isActive() stays
            // true forever — the voice would keep rendering its ringing tail at
            // full envelope AND burning the whole KS/modal DSP path (WR-01). Kill
            // the env (render guard exits) and the circulating energy.
            ampEnv.reset();
            string.reset();
            modal.reset();
            clearCurrentNote();
...
private:
    ...
    //==========================================================================
    // Pitch-bend range: standard ±2 semitones at full wheel deflection (WR-04).
    static constexpr float kBendRangeSemis = 2.0f;

    float computeF0() const noexcept
    {
        // Block-rate bend is enough: setParams re-runs computeF0/setFrequency for
        // active voices every block, so pitchWheelMoved is picked up next block.
        const float bendSemis = kBendRangeSemis
                              * static_cast<float> (pitchWheelPos - 8192) / 8192.0f;
