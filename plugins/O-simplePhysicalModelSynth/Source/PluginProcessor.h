...
public:
    ...
    // Tail is dominated by the RESONATOR ring-out, not the amp release: at
    // decay=100 the KS loop feedback is 0.999 → T60 ≈ ln(1000)/ln(1/0.999) ≈ 6904
    // loop passes ≈ 6904/f0 seconds (≈16 s at A4, longer for low notes); the modal
    // bank reaches T60 ≈ 7.2 s. 30 s covers the practical worst case so hosts that
    // honor tail length (offline bounce / freeze) don't truncate the ring. Very low
    // notes at max decay can exceed this — accepted approximation (WR-05).
    ...
    /** The codec. languageIndex() maps anything it does not recognise to 0, so a
        ...
        v1.3.0 widened both halves to carry Simplified Chinese. The code is the
...
private:
    ...
    // `decay` (so the two knobs visibly co-move). APVTS listeners fire on whatever
    // thread changed the value — host automation arrives on the AUDIO thread, where
    // calling setValueNotifyingHost (host performEdit) is a VST3 violation (WR-02).
    // So: message thread → apply synchronously (preserves preset-load ordering,
    // CR-03); any other thread → stash targets + triggerAsyncUpdate.
    void parameterChanged (const juce::String& paramID, float newValue) override;
    void handleAsyncUpdate() override;
    void applyMaterialMacro();
    std::atomic<float> pendingDamping { 0.0f }, pendingDecay { 0.0f };

    // CR-03 (state path): session restore delivers ALL params explicitly, so the
    // macro must not re-derive damping/decay from the restored material and stomp
    // the restored values. Set around setStateFromXml; checked in parameterChanged.
    std::atomic<bool> restoringState { false };

    // Cached raw-atomic param pointers — looked up once (constructor), read with a
    // relaxed load() per block on the audio thread. Named members, NOT a map: a
    // String-keyed lookup would heap-allocate juce::String temporaries per call in
    // processBlock (CR-01).
    void cacheParamPointers();
    struct RawParams
    {
        std::atomic<float>* excitationType     = nullptr;
        std::atomic<float>* excitationPosition = nullptr;
        std::atomic<float>* excitationColor    = nullptr;
        std::atomic<float>* bowForce           = nullptr;
        std::atomic<float>* resonatorType      = nullptr;
        std::atomic<float>* inharmonicity      = nullptr;
        std::atomic<float>* modeBrightness     = nullptr;
        ...
        std::atomic<float>* velToBrightness    = nullptr;
