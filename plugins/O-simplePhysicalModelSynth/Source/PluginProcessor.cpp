...
{
    using namespace ParamIDs;
    for (auto* id : { excitationType, excitationPosition, excitationColor, bowForce,
                      resonatorType, stringModel, inharmonicity, modeBrightness,
                      damping, decay, material, coarseTune, fineTune,
                      ampAttack, ampRelease, velToBrightness, outputLevel })
        rawParams[id] = parameters.getRawParameterValue (id);
}
...
PhysicalModelParams OSimplePhysicalModelSynthAudioProcessor::readParams() const
{
    ...
    pp.excitationType     = (int)   p (excitationType)->load();
    pp.resonatorType      = (int)   p (resonatorType)->load();
    pp.excitationPosition =          p (excitationPosition)->load();
    pp.excitationColor    =          p (excitationColor)->load();
    pp.bowForce           =          p (bowForce)->load();
    pp.inharmonicity      =          p (inharmonicity)->load();
    pp.modeBrightness     =          p (modeBrightness)->load();
    pp.damping            =          p (damping)->load();
    pp.decay              =          p (decay)->load();
    pp.coarseTune         = (int)   p (coarseTune)->load();
    pp.fineTune           =          p (fineTune)->load();
    pp.ampAttack          =          p (ampAttack)->load();
    pp.ampRelease         =          p (ampRelease)->load();
    pp.velToBrightness    =          p (velToBrightness)->load();
    return pp;
}

//==============================================================================
// Material macro (message thread). material ∈ [0,100]: 0 = steel (bright/long),
// 100 = nylon (dark/short). Writes BOTH visible knobs so they truthfully track
// the macro (RESEARCH §2.4). Only fires on `material` changes → no recursion.
void OSimplePhysicalModelSynthAudioProcessor::parameterChanged (const juce::String& paramID, float newValue)
{
    ...
    dampingPct = juce::jlimit (0.0f, 100.0f, dampingPct);
    decayPct   = juce::jlimit (0.0f, 100.0f, decayPct);

    if (auto* dp = parameters.getParameter (ParamIDs::damping))
        dp->setValueNotifyingHost (dp->convertTo0to1 (dampingPct));
    if (auto* dc = parameters.getParameter (ParamIDs::decay))
        dc->setValueNotifyingHost (dc->convertTo0to1 (decayPct));
}
...
void OSimplePhysicalModelSynthAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    outputGain.setCurrentAndTargetValue (
        juce::Decibels::decibelsToGain (p (ParamIDs::outputLevel)->load()));
}
                                                            ...
                                                            juce::MidiBuffer& midiMessages)
{
    ...
    outputGain.setTargetValue (juce::Decibels::decibelsToGain (p (ParamIDs::outputLevel)->load()));
