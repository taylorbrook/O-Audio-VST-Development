...
- **The hover-help switch, in the settings gear beside the language selector.**
  Through v1.1.1 the gear panel held the language row alone — its comment said
  ...
  localStorage `osbm.tipsEnabled` (default on) — no C++ state, no bridge, no parameter.
...
- `tests/render-harness/CMakeLists.txt`: `JucePlugin_VersionCode` 0x10101 →
  0x10200, following the version (the fixture drifted once before, on O-simpleFM).

### Validation
- check-i18n: 15/15 PASS, 0 / 85 entries unreviewed (30 tooltip, 55 label).
- i18n-fr-lint: CLEAN, exit 0.
- boot-all-uis `--strict-tips`: clean, 0 dead / 0 late tip bindings.
- check-ui-labels: ALL CHECKS PASSED; 92 of 92 `[data-i18n]` elements visible.
