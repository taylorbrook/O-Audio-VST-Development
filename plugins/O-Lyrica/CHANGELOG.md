...
- **The stale width note in `js/i18n.js` re-measured, not scaled.** It cited the
  superseded caption's rendered width and would have become false the moment the
  caption changed. From `check-ui-labels --plugin O-Lyrica --verbose`:
  `Infobulles` renders **45.55 px** against the old 65.89, so the settings row
  reads `45.55 + 12 gap + 40 toggle` in a 154 px content box — **56.45 px of
  slack**, up from 36. The popover holds 178 px and the toggle's left edge does
  not move. `Infobulles` is 4.17 px NARROWER than English `Hover help` (49.72),
  reversing a caption that was 16.17 px wider.
