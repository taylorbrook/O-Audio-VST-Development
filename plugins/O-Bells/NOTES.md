...
- **2026-09-20 (v4.8.0):** Preset-differentiation Step 5 (last) — all 25 factory presets re-voiced, same 5 categories and 25 names; no parameter / state change. RC-3: Damping was authored inverted (Large Bells 0.88–1.0 → 0–0.15; bars / plates → 0.75–1.0). Every preset picks a Partial Model (Classic 11 / Tubular 4 / Plate 4 / Bowl 3 / Glass 3) and sets Hum / Prime Level; RC-5: each has its own chorus / delay / EQ / reverb-shape signature (none had any). NO preset names Output Gain (the user's control; gated) — the bank is balanced by voicing: 8.7 dB RMS spread (old bank 8.5), loudest single-note peak −8.1 dBFS. Factory sentinel 4.1.1 → 4.8.0 (bank regenerates in place). Harness, voice-isolated: pair median 13.3 → 25.1 tap / 14.7 → 25.6 held; worst nearest neighbour 3.5 → 10.6 over both seeds (gate ≥ 8); tap T40 medians Large 9.94 > Warm 5.56 > Bright 4.18 ≥ Clanging Plate 1.45 (Large Bells were the SHORTEST category before). `report.py` gained the bank gates + a stale-bank guard (it removes `.factory_version` each run — the harness reads the INSTALLED plugin's factory directory). auval + pluginval 5 pass. **Hands-on listen in the Standalone: pending (human gate).**
