...
- **2026-09-20 (v4.8.0):** Preset-differentiation Step 5 (last) — all 25 factory presets re-voiced, same 5 categories and 25 names; no parameter / state change. RC-3: Damping was authored inverted (Large Bells 0.88–1.0 → 0–0.15; bars / plates → 0.75–1.0). Every preset picks a Partial Model (Classic 11 / Tubular 4 / Plate 4 / Bowl 3 / Glass 3) and sets Hum / Prime Level; RC-5: each has its own chorus / delay / EQ / reverb-shape signature (none had any). Output Gain per preset balances the bank to −24 dBFS RMS ±0.8. Factory sentinel 4.1.1 → 4.8.0 (bank regenerates in place). Harness, voice-isolated: pair median 13.3 → 26.0 tap / 14.7 → 26.6 held; worst nearest neighbour 3.5 → 10.5 over both seeds (gate ≥ 8); tap T40 medians Large 9.97 > Warm 5.56 > Bright 3.96 ≥ Clanging Plate 1.44 (Large Bells were the SHORTEST category before). `report.py` gained the bank gates + a stale-bank guard (it removes `.factory_version` each run — the harness reads the INSTALLED plugin's factory directory). auval + pluginval 5 pass. **Hands-on listen in the Standalone: pending (human gate).**
