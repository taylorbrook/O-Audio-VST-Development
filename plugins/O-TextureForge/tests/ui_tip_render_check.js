...
(async () => {
    ...
    // and no repo-wide census can see it, because a call site spells no list to
    // grep for.
    //
    ...
    check(LANGUAGES.length >= 2 && LANGUAGES[0] === 'en',
        `[0] LANGUAGES is derived from the table and holds ${LANGUAGES.length} entries — `
        + `${LANGUAGES.join(', ')}`);
    ...
    //
    // The list is DERIVED. This loop replaces a hand-written call, which was the
    // second of this gate's two language sites.
