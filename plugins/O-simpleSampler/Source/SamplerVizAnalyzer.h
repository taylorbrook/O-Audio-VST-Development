...
/*
  ==============================================================================
    ...
    O-simpleSampler - Visualization Tap (real-time-safe spectrum + oscilloscope)
    ...
    Two pieces:
      ...
      - SamplerVizAnalyzer : message-thread FFT (4096 / Blackman-Harris) + scope
    ...
    Phase 2.3: ported VERBATIM from O-simpleGrain (Source/VizAnalyzer.h, itself a
    copy of O-simpleFM's FmVizAnalyzer). Ring + FFT logic UNCHANGED — only the
    analyzer type was renamed GrainVizAnalyzer -> SamplerVizAnalyzer (all behavior
    identical). The audio thread only WRITES output samples to the ring; the FFT
    is the Stage-3 editor Timer's job. The Stage-3 sampler editor additionally
    overlays the lead-voice playhead (PluginProcessor::getDisplayPlayhead) on the
    waveform — the ring write here is independent of that.
...
//==============================================================================
class SamplerVizAnalyzer
{
public:
    static constexpr int kFftOrder    = 12;          // 4096 — separates discrete partials
