...
// The order is load-bearing: member zero is the SOURCE language every other
// member is authored from and compared against, by applyI18n(), by
// check-ui-labels and by this plugin's own tests/ui_tip_render_check.js,
// which derives its sweep from this array rather than carrying a copy.
//
// v1.9.0 adds Simplified Chinese. The 37 shared tuning-panel captions gained
// their column in the SAME change as this line, deliberately: a language
// column under a two-member declaration is reachable by no page and reported
// by no gate in this repo, so landing the two separately would ship a silent
// partial rather than a caught defect.
...
export const I18N = Object.freeze({
    ...
    // ── The gear ────────────────────────────────────────────────────────────
    //
    // THIS BODY DESCRIBES ONLY WHAT THE POPOVER ACTUALLY HOLDS, AND IT WAS
    // WRONG FROM v1.7.0 UNTIL v1.9.0. Through Stage M the popover really did
    // hold one control and the body said so. v1.7.0 then put #tips-toggle
    // inside .settings-popover — index.html, the second .settings-row beside
    // #lang-select — and neither this body nor this comment was revisited, so
    // both shipped a falsehood for two releases, in English and in French.
    // check-i18n's repo-wide hover-help clause has been passing for this
    // plugin the whole time, which is the independent proof the switch exists
    // and the copy was the thing that was wrong. Corrected here to name both
    // controls. And it opens DOWNWARDS here — .settings-popover is
    ...
    'tip.settings': {
        en: { t: 'Settings',
              b: 'Opens the settings panel below this button. It holds the interface language '
               + 'and the switch that turns this hover help on and off. Press Escape to close it.' },
        fr: { t: 'Réglages',
              b: 'Ouvre le panneau de réglages sous ce bouton. Il contient la langue de '
               + 'l’interface et le commutateur qui active ou désactive ces infobulles. '
    ...
    //
    // The selector's option words are ENDONYMS in the markup — a language name
    // is never translated. They are not AudioParameterChoice options, so D-01
    // arm 1 is not in play. The body no longer NAMES them: a body that recites
    // the selector's contents has to be rewritten every time the selector
    // grows, and it was already wrong once. The markup is the list.
    //
    // THE TUNING CLAUSE WAS LOAD-BEARING AND IT IS NOW FALSE — CORRECTED HERE.
    // It read that this plugin's Tuning page stays English because the panel
    // comes from the shared module (CMakeLists.txt embeds
    // modules/tuning/scala-tuning-engine/js/tuning-panel.js by reference), and
    // that was true through v1.7.0. The module was localized at v3.1.0 and
    // this plugin's own v1.8.0 shipped all 37 of its caption rows, so from
    // that release the clause was a shipped falsehood — in French as well as
    // English. The Tuning page follows the selector like everything else, so
    // it is simply out of the exception list. What is left of that list is
    // true: the value readouts really do stay English, because they are
    // formatted by code and carry no table row.
    'tip.language': {
        en: { t: 'Language',
              b: 'Chooses the language of every caption, tooltip and accessible name on this '
               + 'panel, and the choice is saved with the plugin. Value readouts stay in '
               + 'English whichever language is chosen.' },
        fr: { t: 'Langue',
              b: 'Choisit la langue de tous les libellés, info-bulles et noms accessibles de ce '
               + 'panneau ; le choix est enregistré avec le plugin. Les valeurs affichées '
...
export const I18N_EXEMPT = [
    ...
    // ── Endonyms ────────────────────────────────────────────────────────────
    // The <option> words in #lang-select. A language names itself in its own
    // script, so a reader looking for their language finds it whatever page
    // they are on. They are markup, not table rows, and they are identical in
    // every language, which is what keeps the select's box out of the
    // geometry diff.
