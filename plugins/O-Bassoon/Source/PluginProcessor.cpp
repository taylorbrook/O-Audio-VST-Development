...
{
    // v1.1.0: the UI language rides the same tree as one more plain property.
    ...
    // — getStateAsXml() starts from parameters.copyState(), so a property set
    // on parameters.state is carried into the XML with everything else.
    //
...
void OBassoonAudioProcessor::setStateInformation (const void* data, int sizeInBytes)
{
    ...
    // v1.1.0: the UI language, read back AFTER the preset manager has restored
    ...
    // getStateInformation writes a STRING var, but even a bool or an int
    ...
    // (critical_valuetree_xml_roundtrip_loses_type). A pre-1.1.0 session has no
