...
public:
    ...
    // ------------------------------------------------------------------------
    // v1.14.0 — the hover-help language. 0 = en, 1 = fr.
    //
    ...
    /** The codec. Three branches as of v1.16.0: "fr", "zh-Hans", English.
        languageIndex() maps anything it does not recognise to 0, so a
        hand-edited session or an unexpected argument from the page degrades to
        English rather than being stored unvalidated.

        Both halves are PURE ASCII, and deliberately so. The BCP-47 subtag is
        the one spelling of a language that crosses the whole C++/JS boundary;
        no endonym, and no Han codepoint, ever enters this file. */
    static juce::String languageCode  (int i)
    {
        return i == 1 ? "fr" : i == 2 ? "zh-Hans" : "en";
    }
    static int          languageIndex (const juce::String& s)
    {
        return s == "fr" ? 1 : s == "zh-Hans" ? 2 : 0;
    }
