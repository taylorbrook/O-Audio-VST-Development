...
- **2026-09-25 (v1.13.0):** Gain reduction now reads off the analyzer. Four regions span the spectrum between the crossovers, each filling downward from 0 dB at 12 dB full scale with a 500 ms / 24 dB-per-second peak line; bypassed and solo-muted bands are hatched, since both already store GR 0 and were otherwise indistinguishable from a band that simply was not compressing. **Drawn as DOM, not into the spectrum canvas, and that is the decision worth remembering here.** `sendSpectrumData()` returns early unless the FFT has new data, so the canvas redraws at FFT cadence while gain reduction arrives on every 30 Hz tick — sharing the canvas would have coupled the two rates and put new code in the one path whose failure mode is a frozen analyzer. No C++ changed at all: the 30 Hz `updateGainReductionMeters` push already carried everything, and this is a second consumer of it. The regions move from `updateBandRanges()` rather than from the drag handler, which is the v1.4.0 lesson applied — the drag is not the only thing that moves a crossover, and hooking it alone leaves host automation and preset loads behind. Second thing worth remembering: the strip readout first shipped in this release at `opacity: 0.45` as de-emphasis, and `measure-ui --contrast` read the composite at 1.32:1 against a 4.5 need. A resting opacity IS a contrast change, and the tool composites it — the fix is the cream chip `.crossover-label` already uses, which also makes the backing deterministic instead of "whatever the `.gr-fill` gradient is under it" (7.95:1 empty, 3.35:1 over the olive stop). 8.73:1 after, page-wide AA shortfalls 339 → 327, zero contributed by this release. Zero new i18n keys — every new string is digits plus `dB`. `check-ui-labels` and `check-i18n` pass.
- **2026-09-14 (v1.12.2):** `updateSidechainFilters` clamped its corner against an absolute 20 kHz "Off" ceiling but never against Nyquist, so at any rate below 40 kHz a band whose SC LPF sat between Nyquist and 20 kHz built a detector biquad with poles at |z| = 1.32. Found while verifying the identical code in O-Comp v1.10.1. **The two plugins present the same defect completely differently, which is the part worth remembering.** O-Comp has no guard in its envelope, so the divergence propagates and the plugin outputs digital silence. Here, `EnvelopeDetector::processSample`'s v1.6.1 non-finite guard rewrites the runaway value to zero before anything can observe it: the output stays finite, no band slams shut, nothing is NaN by the time it is measurable — the band just stops seeing its own signal and quietly gives up compressing. A "finite output" gate and a "gain reduction within sane bounds" gate were both written first and both went green on the broken build. The gate that works discriminates against the *clamp target*: 0.45 × 32 kHz is 14.4 kHz, so with the clamp "18 kHz" and "14.4 kHz" are the same request and must render identically, and without it all four bands move by up to 7 dB. The guard was doing its job; it was hiding a defect upstream of it.
