...
private:
    ...
    // Async Scala/TUN file picker (native fn "openTuningFilePicker").
    // SafePointer + bare-return-on-null pattern — see .cpp for the UAF contract.
    ...
    // Owns the in-flight async chooser (must outlive launchAsync).
    std::unique_ptr<juce::FileChooser> tuningFileChooser;
