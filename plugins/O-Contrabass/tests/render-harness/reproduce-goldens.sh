#!/usr/bin/env bash
# Phase 2.4a R34-pre / Phase 2.4b R35a — canonical reproduction of all 12
# O-Contrabass render goldens. Locks invocations against duration-dependence
# trap (RESEARCH.md §17.10 Risk #10). Phase 2.4b extension (PLAN rev-9 pin #3):
# adds `sub-harmonics` + `sub-harmonics-stability` entries; HR-9 IEEE 754
# identity arithmetic preserves bit-exact regression for the 10 carry-forward
# goldens at SUB_HARMONICS=0 default.
...
NAMES=(stiffness-zero-pre string-A string-D string-G detune-sweep-A note-sequence vibrato macro-sweep slow-lfo schelleng-stress sub-harmonics sub-harmonics-stability)
INVOCS=(
