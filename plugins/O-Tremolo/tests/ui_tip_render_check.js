...
/*
  ==============================================================================
    ...
    ui_tip_render_check.js — O-Tremolo hover-help, RENDERED.
    ...
    NO OTHER GATE IN THIS REPO CAN SEE A RENDERED TOOLTIP.
      ...
      check-i18n      reads the table statically. Its assertion 2 counts
                      ...
                      Eight bindings pointing at eight selectors that match
    ...
    So authoring eight tooltip bodies into i18n.js and binding them, with no
    other change, would have shipped eight invisible strings past three green
    gates. v1.7.0 of this plugin had NO #tooltip node, NO .tooltip rule and NO
    hover handler, which is exactly the state in which that failure is silent.

    THIS PLUGIN HAS THE SCAR ALREADY. v1.6.0 shipped English section headers
    inside a French preset dropdown with every gate green, because
    `header.textContent = headerText` put the English literals one frame away at
    the addSection() call site and check-ui-labels assertion 12 is blind to that
    shape. A gate that certifies the absence of a thing it cannot see is the
    failure mode this file exists to close for hover-help.
    ...
    ── THE FRAME IS 600 x 400, READ NOT GUESSED ────────────────────────────────
    ...
    (pattern_tooltip_clamp_gate_viewport_sensitive). The gear sits 6 px from the
    ...
    Usage:
        ...
        node plugins/O-Tremolo/tests/ui_tip_render_check.js --verbose
...
async function loadTable() {
    ...
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'otremolo-i18n-'));
...
(async () => {
    ...
    try {
        ...
        if (surface && typeof surface === 'object') {
            ...
            // surface leaves check-ui-labels byte-identically green (a fixed
            // box at 0,0 has the same rect in both languages, so it neither
            ...
            // #lang-select is itself an anchor and lives INSIDE
            // .settings-popover (z-index 61), so a tip that did not out-stack
            // the panel would open behind the control that opened it.
        ...
        // ── the driving loop ────────────────────────────────────────────────
        //
        // The six parameter anchors are hovered with the settings popover
        ...
        for (const lang of ['en', 'fr', 'en']) {
            ...
            for (const group of [{ open: false, list: anchorsClosed }, { open: true, list: anchorsOpen }]) {
                ...
                for (const sel of group.list) {
                    ...
                    // A .tip-title that silently kept the PREVIOUS anchor's
                    // text passes a contains check on a body that mentions the
                    // same words. Equality is the only test that catches it.
        ...
        //
        // The plant is TALL, not wide: max-width 240px means a long body wraps
        // rather than running off the right edge, so the reachable overflow on
        // this renderer is vertical. A body that cannot fit in 400 - 16 px of
        // frame is one the clamp is unable to place, and assertion 4 must say
        // so.
        console.log('\n-- negative control (assertion 4 harness-blindness)');
        const PLANT = ('overflow probe. ').repeat(60);
        await page.evaluate((body) => {
            const el = document.querySelector('#depthKnob').closest('.knob-container');
            el.setAttribute('data-tip', body);
        }, PLANT);

        const planted = await hoverAnchor('#depthKnob', '.knob-container');
        ...
        await page.evaluate(() => window.__setLanguage('en'));
        await page.waitForTimeout(150);
        const restored = await hoverAnchor('#depthKnob', '.knob-container');
        ...
        // therefore re-opens the tip that pointerdown just hid, with the
        // pointer still on the anchor and no further pointerover coming — and
        // the tip sits on top of whatever the click opened. Measured on the two
        // sibling plugins that landed without the latch: the gear's tip covered
        // the settings popover by 146 x 35 px on O-Bass and 161 x 29 px on
        // O-AnalogSaturation.
        //
