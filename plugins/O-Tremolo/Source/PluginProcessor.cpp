...
{
    // v1.7.0: the UI language rides the same tree as one more plain property.
...
void OuariconTremoloAudioProcessor::setStateInformation(const void* data, int sizeInBytes)
{
    ...
    // v1.7.0: the UI language. Read AFTER setStateFromXml, which calls
    ...
    // (critical_valuetree_xml_roundtrip_loses_type). A pre-1.7.0 session has no
