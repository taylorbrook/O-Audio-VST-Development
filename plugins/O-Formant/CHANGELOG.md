...
- **The second row costs nothing, but it had to be a SIBLING to cost nothing.** This popover is `display: block`, and the language caption and its `<select>` sit inside one `.settings-row`. Dropped in after the `</select>` the new row landed INSIDE that row, and `check-ui-labels` [7] reported four moved non-label elements — `#settings-popover` dx=-4.0 dw=+4.0 and `#lang-select` dx=-15.0 — because a French caption inside a space-between row pushed the panel's own `min-width: 168px` box wider. Reparented as a sibling with a `.settings-row + .settings-row { margin-top: 6px }` rule (a gap on the parent does nothing on a block container), the panel measures **y 34..117, 168 x 83 px — byte-identical in English and French** in an 800 x 600 frame, and [7] reports **0 moved**.
...
- `tests/ui_tip_render_check.js` [8] counts **eleven** non-`[data-param]` anchors,
  not ten — `#tips-toggle` joined `#gear-btn` and `#lang-select`. The number
  still names what it counts rather than being a bare literal.
