...
- **IN-17** Partch 43-Tone has 41 entries (`EmbeddedTunings.cpp:130-133`) — missing 11/10 and 20/11. *(O-Formant's table only; the scala-tuning-engine module and the other plugin copies are a separate follow-up.)* **Resolved in v1.32.2**
- **IN-18** Tonic semantics differ: linear mode shifts the anchor by 12-TET semitones (`TuningEngine.cpp:885`), KBM mode rotates by scale degrees (`:811, :352`). *(Documented and left as is: `TuningEngine.h` setTonicNote.)* **Resolved in v1.32.2**
...
| v1.32.1 | `4bc14ee2` | IN-12 (stage-0 diffusion wrap) |
| v1.32.2 | _pending_ | IN-17, IN-18 (documented), IN-24a/b/c (tabs, gear, generated scale names) |

Closed without change: IN-22b (drag listeners already on `document`), IN-23c (overlay adds 0 px — measured), IN-24f (UI shows `ct`).

Still open: IN-10, IN-21 (navigation), IN-24 (knob focus, wheel, double-click reset, fine drag).

Chosen resolutions for the next sweep (decided 2026-09-25; applied in v1.32.2): IN-17 — fix O-Formant's table only (add 11/10 = 165.0 ¢ and 20/11 = 1035.0 ¢; the scala-tuning-engine module and the O-Bells/Prism/Strata/Lyrica/IntonationPad copies as a separate follow-up); IN-18 — document and leave; IN-24 — semantics + i18n only (tab role/tabindex, gear `aria-expanded`, localized generated scale names); knob focus, wheel, double-click reset and fine drag stay open.
