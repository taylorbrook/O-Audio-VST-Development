...
Kept for history and for anything worth lifting into v1.1's baked source type: the ARCHITECTURE (mesh slicer algorithm, scheduler contract, persistence design), ROADMAP, locked parameter-spec v1 (219 params), mockup v1 and its scaffolding, and the Stage 0 / Stage 2 CONTEXT files. `../stages/1-foundation/first-pass-baked/` is **not** superseded — it records the fork that exists in `Source/` and is the base for the re-parameterise pass (whose own artifacts live in `../stages/1-foundation/`).
