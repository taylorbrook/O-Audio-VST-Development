...
Options: `--gate G` (repeatable; `H1..H11 | tuning | smoke | centroids | saturation | decimator | crossfade | latency | clenshaw | scheduler | storm | import | export | topnote | orbits | h2cli | all` — `export`, `orbits` and `h2cli` are never part of `all`), `--out PATH` (the `--gate orbits` JSON, default `./orbits.json`), `--dump-choices` (prints the combo choice lists + mod-matrix names as JSON and exits — the source of `tests/ui-stub/generic-overrides.json`, Stage 3), `--note N`,
