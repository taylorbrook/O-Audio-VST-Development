...
/*
  ==============================================================================
    ...
    ui_tip_render_check.js — O-Strata hover-help, RENDERED.
    ...
    ── THE PAGE IS A FOUR-TAB DECK, AND THREE TABS ARE ZERO-SIZE AT LOAD ──────

    `.tab-content` is `display: none` unless `.active` (index.html:317-325), so
    #mod-tab, #tuning-tab and #effects-tab cannot be hovered or
    ...
    from a namespaced copy under scratchpad/O-Strata/m3/ — never with
...
async function loadTable() {
    ...
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'ostrata-i18n-'));
...
(async () => {
    ...
    try {
        ...
        async function openTab(name) {
            ...
            // Click the tab's LEFT edge, not its centre. This gate keeps the settings
            // popover OPEN while it walks the tabs (see assertion 1), and the popover
            // (right: 0, 168px) hangs over the rightmost tab's right-hand part. With
            // O-Prism's five tabs that was the Wavetable tab, which this loop never
            // clicks; with O-Strata's four Stage-1 tabs it is Effects (900-1200 px at
            // the 1200 px frame, centre 1050 > 1032), so a centre click is intercepted
        ...
        // is the wrapper walk — 21 `#select-*` rows walk to `.dropdown-group`,
        ...
        // was also taken at scratchpad/O-Strata/m3/ before this run, per the
