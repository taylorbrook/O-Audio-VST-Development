    ...
    : AudioProcessorEditor(&p), audioProcessor(p)
{
    ...
    thresholdAttachment = std::make_unique<juce::WebSliderParameterAttachment>(
        *audioProcessor.parameters.getParameter("threshold"), *thresholdRelay, nullptr);
    ratioAttachment = std::make_unique<juce::WebSliderParameterAttachment>(
        *audioProcessor.parameters.getParameter("ratio"), *ratioRelay, nullptr);
    attackTimeAttachment = std::make_unique<juce::WebSliderParameterAttachment>(
        *audioProcessor.parameters.getParameter("attack_time"), *attackTimeRelay, nullptr);
    releaseTimeAttachment = std::make_unique<juce::WebSliderParameterAttachment>(
        *audioProcessor.parameters.getParameter("release_time"), *releaseTimeRelay, nullptr);
    kneeAttachment = std::make_unique<juce::WebSliderParameterAttachment>(
        *audioProcessor.parameters.getParameter("knee"), *kneeRelay, nullptr);
    ...
    autoGainAttachment = std::make_unique<juce::WebToggleButtonParameterAttachment>(
        *audioProcessor.parameters.getParameter("auto_gain"), *autoGainRelay, nullptr);

    // Send initial parameter values to UI
    thresholdAttachment->sendInitialUpdate();
    ratioAttachment->sendInitialUpdate();
    attackTimeAttachment->sendInitialUpdate();
    releaseTimeAttachment->sendInitialUpdate();
    kneeAttachment->sendInitialUpdate();
    ...
    // Set window size from YAML dimensions
    setSize(620, 360);
}
