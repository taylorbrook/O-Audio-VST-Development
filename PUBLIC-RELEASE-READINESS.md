...
### 4.4 Root-level scratch files and first impressions [E4] — ✅ RESOLVED 2026-08-03

**As assessed 2026-07-30 —** these were untracked, so they would not have been published, but they sat in the working directory and made the repo root read as a scratch space rather than a project: `e1-max-sustain.wav` (17 MB), `o-bowed-pre-extraction-canonical.wav` (1.3 MB), `mbc-v150.png`, `o-reversedelay-484.png`, `tooltip-knob.png`, `.DS_Store`, and `scratch-pv/` [E4].

**Resolved 2026-08-03.** The root now holds no loose render, screenshot, Finder metadata file, or tool-output directory. Grouped by what actually happened to each item rather than listed flat:

- **One file was relocated inside the repository.** `o-bowed-pre-extraction-canonical.wav` (1,323,104 B) moved to `plugins/O-Bowed/tests/render-harness/golden/canonical-preset.wav`. See the sub-point below — this is the one item in the pile that was load-bearing.
- **Everything else was copied out to a dated archive before it was removed:** `~/VST-development-root-scratch-20260803/`, holding the `e1-max-sustain` wav+json pair, six PNGs, the six `scratch-pv/` pluginval log dumps, the root o-bowed harness JSON, and the `.playwright-mcp/` tree. All **77** files were MD5-compared against their originals and matched **before** anything was deleted; the preservation gate was proven, not assumed. A `README.txt` in that directory records the provenance of each group so it is intelligible without the repo.
- **One file was deleted outright:** the root `.DS_Store` (6,148 B) — the sole item in this tidy with no preservation copy. Finder metadata carries no content, regenerates on its own, and cannot be referenced by anything.
- **Nothing here was ever tracked**, so unlike steps 5, 6, 7 and 12 this was a pure working-directory tidy: no `git rm --cached`, no index change, and no history implications whatsoever.

**Correction to this section's own record — the root `e1-max-sustain` pair.** This section listed it as plain scratch alongside the screenshots. It is more specific than that, and the distinction matters because a **tracked** document cites it. `plugins/O-Contrabass/.planning/stages/2-dsp/RESEARCH.md` line 1390 names "`e1-max-sustain.json` (project root, captured at 2026-04-26 verify-phase rerun)" as its concrete numerical evidence, quoting `rmsMid_s5_s6 = 0.0353`, `rmsFinal_lastSecond ≈ 0`, `pass_rms = false`. **That "(project root)" parenthetical is wrong.** Measured 2026-08-03, the root file does not carry those numbers — the tracked plugin-local pair does:

| Field | ROOT pair (archived) | PLUGIN-LOCAL pair (tracked, `plugins/O-Contrabass/`) |
|---|---|---|
...
`plugins/O-Contrabass/.planning/stages/2-dsp/VERIFICATION.md` line 68 reproduces "peak 0.0683, rmsMid 0.0353, rmsFinal 0.0127, ratio 0.36". The plugin-local pair matches that on `rmsFinal` and `rmsRatio` to the digit and on `peak` and `rmsMid` to within the drift between two re-runs of the same harness invocation. The root pair matches on **no field at all**. The two wavs are the same byte length (17,199,104 B) but different content — different runs, not a copy: md5 `ba8d5b37ba5fc613b00dbe313c4f00e5` (root) versus `d585110e4f7738a15aaca397c17c0fe1` (plugin-local).

**So: the cited evidence artifact is the plugin-local pair.** It is tracked, it is untouched by this tidy, and it survives regardless. The root pair is a superseded harness run whose numbers appear in no document — and it was archived rather than deleted *precisely because* a tracked document names it, however inaccurately. **`RESEARCH.md` itself was deliberately not edited.** It is a historical stage document; correcting the record belongs here, not in a rewrite of the record.

**Why the O-Bowed render was relocated and then ignored rather than committed.** Its sha256 is `93124fb8dd8223caafac5948c988a226230363d79a17323d386e9a1db34c8891`, matching `plugins/O-Bowed/tests/render-harness/golden/canonical-preset.wav.sha256` exactly. That golden directory has only ever tracked the `.sha256` sibling, never the `.wav`, so the root file was the **only byte-exact copy of the render on disk anywhere** — deletion was never an option, and the checksum that vouches for it is where it belongs. But the destination is a directory where renders *are* legitimately tracked: O-Contrabass keeps nineteen of its own. A stray wav there looks native and would have been swept up by one `git add -A` into a now-public repo. **The ignore rule therefore landed and was committed before the move**, so the binary was never simultaneously present and unignored — the same window that reopened step 6, designed out rather than fixed after the fact. The rule at `.gitignore:68` names that one exact path and not a directory wildcard, because a broader rule would silently stop future intentional goldens from being tracked. Not committing the binary is also the right default on size grounds: **§4.2** identifies the tracked audio goldens as the largest single contributor to repository size and **step 14** exists to reclaim exactly that, so adding another multi-megabyte binary would work against the standing direction.

**What was considered and ruled out of scope.** `backups/` (1.3 GB) and `logs/` (9.8 MB) were looked at and **deliberately left untouched** — they are established working directories, not loose renders or images, and `logs/`'s build logs were already handled by step 7. They were not missed. `build/` and `build-release/` likewise stay as-is; step 6 closed the latter. Two groups also went beyond this section's original list and were decided **explicitly rather than swept in silently**: the three before/after UI screenshots from 2026-08-02 (`after-header.png`, `after-tips-off.png`, `before-full.png`) and the `.playwright-mcp/` browser-tool output directory. Both postdate the 2026-07-30 assessment, both are the same class of artifact, and both were archived under the same uniform rule — **nothing was deleted in this tidy except Finder metadata**, so every disposition is reversible.
...
- [x] ~~**13. Tidy the repo root** — the scratch renders and images that make the root read as a workspace.~~ ✅ **Done 2026-08-03** — the root now holds no loose render, screenshot, Finder metadata file, or tool-output directory. *(Section 4.4 — [E4].)* **The preservation gate ran before anything was removed.** The `e1-max-sustain` wav+json pair, six PNGs, the six `scratch-pv/` pluginval logs, the root o-bowed harness JSON, and the `.playwright-mcp/` tree were all copied to `~/VST-development-root-scratch-20260803/` and MD5-matched against their originals — **77 of 77** files — *before* a single original was deleted; a `README.txt` there records what each group is and what is safe to discard. Only one item was deleted outright: the root `.DS_Store` (6,148 B), the sole file in this tidy with no preservation copy. **One file was load-bearing and moved rather than archived.** `o-bowed-pre-extraction-canonical.wav` was the only byte-exact copy on disk of O-Bowed's golden render — sha256 `93124fb8dd8223caafac5948c988a226230363d79a17323d386e9a1db34c8891`, matching the tracked `.sha256` sibling that attests it — so it was relocated to `plugins/O-Bowed/tests/render-harness/golden/canonical-preset.wav` and re-verified byte-exact after the move. **Step 6's window never opened:** the ignore rule landed and was committed *before* the file was moved into place, so the binary was never simultaneously present and unignored — which mattered here because the destination is a directory where renders *are* tracked (O-Contrabass keeps nineteen). The rule at `.gitignore:68` names that one exact path, never a directory wildcard. Verified: `git check-ignore -v` resolves the destination to that rule; the tracked-but-ignored census holds at **9** (all vendored `preset-manager.js`); the tracked `.wav` total holds at **31**; O-Contrabass's golden directory still tracks **70** files of which **19** are renders; and a whole-repository dry-run `git add -An` stages **zero** paths. **Nothing here was ever tracked**, so unlike steps 5, 6, 7 and 12 this was a working-directory tidy with no untracking, no index change, and no history implications at all. Left alone deliberately: `backups/` (1.3 GB) and `logs/` (9.8 MB) are established working directories rather than scratch output — considered and ruled out of scope, not missed — as are `build/` and `build-release/`. One record correction landed alongside: §4.4 had listed the root `e1-max-sustain` pair as plain scratch, but a tracked `RESEARCH.md` cites it as numerical evidence and **that citation is stale** — the real artifact is the tracked plugin-local pair, and the root pair was archived rather than deleted precisely because a tracked document names it.
