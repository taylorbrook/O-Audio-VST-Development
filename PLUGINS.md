...
| O-Strata | 🚧 Stage 4 Polish — Round A verified 2026-09-13 (`stages/4-polish/round-a/VERIFICATION.md` ✅ VERIFIED (Round A, automated): every Task 10 gate re-measured from `29803cb5` — `--gate all` 159 / 159 in 111.4 s, H2 20 / 20 preset rows, H10 24 / 24 (two 1.9 MB slots in 64 ms; a missing source plays Sine Product), smoke --with-disk 39 / 39, labels 1809 / 0 over default + 42, tips 3215 at 122, layout 607 / 0, shell diff 0 px, lints 0 with 53 'mt', orbit golden 44 / 44, params diff empty, 0 new warnings, pluginval ×2 + auval on the installed `-dev` bundles; FUNC-08 / FUNC-11 / QUAL-03 complete; the Squarcle Storm H2 figure varies run-to-run through the S&H LFO's clock-seeded RNG (always inside −6 dB) → Round B item; six human rows open). Round B (Phase 4.2) plan ✓ 2026-09-13 (`stages/4-polish/PLAN.md` — 12 tasks / 4 waves / 3 commits, Decisions 27–45: portable padded Chebyshev evaluator gated by `--gate clenshaw` maxΔ ≤ 2e-5 / H6 ± 0.5 dB / H7 Bandlimited ≤ 2× + 2 %; S&H LFO seeded under the harness seed; `ci-tests.yml` `plugin` dispatch input with O-Strata macOS harness + Windows VST3 / pluginval-10 steps, pushed + dispatched once; PERF-03 burst by script; LISTENING.md Tables A / B / C + 18 preset renders; CHANGELOG v1.0.0 / NOTES attribution / 📦 Installed; no tag) — execute next | 1.0.0 | Synth (Microtonal Wave Terrain) | 2026-09-13 |
...
| O-ReverseDelay | 📦 Installed | 1.12.0 | Audio Effect (Granular Reverse Delay) | 2026-09-06 |
| O-Octagon | 📦 Installed | 1.13.0 | Audio Effect (8-Channel DBAP Spatializer) | 2026-09-08 |
